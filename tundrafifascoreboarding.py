...
def tundrafifascoreboarding():
  #This is purely for creating a CSV file that is empty
  
  
  
  f = open('Scoreboard20.csv', 'w+')
  
 
  f.close()
  tundraupload_file('/fifascoreboard.csv', 'Scoreboard20.csv')
  



def tundrafifascoreboardreader(pagenumber):
  #opens the scoreboard file + generates a file to store a sorted list for leaderboard
  tundradownload_file('/fifascoreboard.csv', 'scoreboard21.csv')
  f = open('scoreboard21.csv', 'r') 
  f2 = open('scoreboard22.csv', 'w') 
...
def tundrafifascoreboardsingle(userID):
  ...
  for row in reader:
    ...
    if(str(row['userIDs']) == str(userID)):
      score = "The Fifa Prediction score for : " + str(row['accountname']) + "  :  " + str(row['score']) + ", giving them rank - " + str(j)
      i=1
  if(i==0):
...
def tundrafifascoreboardadder(usersname, userID, scoretoadd, counter):
  i=0
  filenames = ["accountname", "userIDs", "score"]

  try:
    if(counter == 1):
      table = BeautifulTable().from_csv('scoreboard21.csv', header=False)
    else:
      table = BeautifulTable().from_csv('scoreboard22.csv', header=False)
