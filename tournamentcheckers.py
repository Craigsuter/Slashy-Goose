...
def ValoCheckTourni(channelDataID):
  try:
    #Gets the URL for the tourni
      ...
      if (my_url=="test" or my_url=="none"):
        #If incorrect - it will return an embed telling the user
        embed=discord.Embed(title="NextVT usage - no tournament set currently!", url="https://www.vlr.gg/team/2/sentinels", color=0xf10909)
        embed.add_field(name="Notice",value="There are currently no tournaments set for the command - please ask a Moderator to update this if there is a tournament that you think should be tracked!", inline=False)
        randomval = 0
        return(embed, randomval)
      ...
      try:
          Teams = (Teams1 + " vs " + Teams2)
          nextdotagame = ("" + Teams1 + " vs " + Teams2 + " on " +
                          nextgametime +
                          ", more information can be found at - " +
                          my_url)
          datetimesplit = nextgametime.rsplit(" ")
    ...
    embed=discord.Embed(title="NextVT usage - no tournament set correctly!!", url="https://www.vlr.gg/team/2/sentinels", color=0xf10909)
    embed.add_field(name="Notice",value="An error was hit during the scan, likely caused by incorrect url, please ask a Moderator to check this", inline=False)
    randomval = 0
